...
export async function getBranchComparison(orgId: string) {
  ...
  const branchMetrics = await Promise.all(
    branches.map(async (branch) => {
      ...
      // Fee collection rate - using StudentFeeStructure and FeeInstallment
      ...
      const collectedFees = installments.reduce(
  ...
  return {
    ...
    totals: {
      ...
      avgAttendanceRate: Math.round(
        branchMetrics.reduce((sum, b) => sum + b.attendanceRate, 0) /
          branchMetrics.length,
      ),
      avgFeeCollectionRate: Math.round(
        branchMetrics.reduce((sum, b) => sum + b.feeCollectionRate, 0) /
          branchMetrics.length,
      ),
      totalFeesCollected: branchMetrics.reduce(
        (sum, b) => sum + b.totalFeesCollected,
        0,
      ),
      totalFeesPending: branchMetrics.reduce(
        (sum, b) => sum + b.totalFeesPending,
        0,
      ),
    },
  };
...
export async function getBranchPerformance(
  ...
  const [branch, students, batches, attendanceRecords, fees, complaints] =
    await Promise.all([
  ...
  const totalFees = fees.reduce((sum, i) => sum + i.amount, 0);
  const collectedFees = fees.reduce((sum, i) => sum + i.paidAmount, 0);
...
export async function getOrgStats(orgId: string) {
  const [studentCount, branchCount, staffCount, activeFeesCount] =
    await Promise.all([
  ...
  const monthlyPayments = await prisma.installmentPayment.aggregate({
    where: {
